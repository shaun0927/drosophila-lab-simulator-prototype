...
- `tools/external-evidence-check.js` also rejects proxy evidence, incomplete accepted rows, invalid #27 provenance/design-effect vocabulary, #27 experience-map field mismatches, duplicated or mixed explicit no-experience rows, incomplete #33 player route coverage, player route/fixture or phrase mismatches between the ledger and validation results, accepted SME reviews without review-id-linked clean/dirty/missing-control fixture coverage, SME rating mismatches between the ledger and validation results, inconsistent player/SME decisions, missing accepted-evidence document references, missing Fix/Cut follow-up references in validation results, vague, self-referential, or unresolved Fix/Cut follow-up notes, and follow-up count mismatches.
