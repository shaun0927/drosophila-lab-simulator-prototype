...
External validation preflight guardrail: `node tools/external-validation-preflight-check.js` verifies the sprint plan still requires pre-session commands, launch aids, and anti-proxy/no-closure warnings before #35-#39 evidence collection starts. `node tools/external-validation-preflight-check.test.js` keeps preflight mismatch fixtures in CI.

External validation capture-packet guardrail: `node tools/external-validation-capture-packet-check.js` verifies the raw capture packet still preserves exact-phrase fields, issue-specific packet links, updated verification commands, SME follow-up warnings, and no-closure language. `node tools/external-validation-capture-packet-check.test.js` keeps capture-packet mismatch fixtures in CI.
...
External validation participant briefs: `docs/external-validation-participant-briefs.md` provides copy-ready #35/#36/#37/#38/#39 invitation and session-start language while preserving first-30-seconds player guardrails and preventing answer-key leakage. `node tools/external-validation-participant-briefs-check.js` and `node tools/external-validation-participant-briefs-check.test.js` keep invitation-only material from becoming evidence and reject wrong route URLs or missing SME fixture coverage.
...
| Improve from the analysis | The current guardrail set includes the objective strip, Reviewer #2 repair plans, validation packet, lived-experience packet, R-series status guardrail, external evidence ledger, ledger validator, capture packet, finding decision tree, pending-status doc checks, ready-for-closure status checks, scoped implementation-contract issue forms, executable issue-template contract checks, partial-evidence intake, proxy-evidence rejection, accepted-row completeness checks, player/SME decision consistency checks, accepted-evidence document references, concrete follow-up issue references, follow-up count validation, #27 lived-evidence vocabulary validation, #27 experience-map consistency validation, follow-up self-reference rejection, follow-up disposition validation, follow-up result-summary references, player/SME decision-result consistency validation, player route-result consistency validation, player phrase-result consistency validation, SME review-id fixture-coverage validation, SME rating-result consistency validation, in-app follow-up gate exposure, dynamic follow-up status validation, explicit no-experience row constraints, no-experience conflict rejection, and #33 route-coverage validation | Complete for the proxy-driven iteration loop | Several iterations converted identified gaps into playable/documented affordances and stronger anti-drift automation. These guardrails make false closure harder; they do not provide the missing lived, player, or SME evidence. |
...
- `docs/external-validation-intake-runbook.md` defines the edit order after real `LE-*`, `P-*`, or `SME-*` evidence is collected.
- `docs/external-validation-session-packets.md` provides the #35 raw interview and accepted-row packet before ledger updates.
- `docs/external-validation-execution-tracker.md` records #35 as open with #27 accepted evidence still at 0.
...
- `docs/external-validation-intake-runbook.md` defines the edit order for copying accepted player/SME evidence into validation results, the ledger, status counts, tracker, and issue comments.
...
- `docs/external-validation-execution-tracker.md` records #36 through #39 as open with #33 accepted evidence still at 0.
...
- `tools/external-evidence-check.js` verifies that ledger counts, in-app status counts, and pending/open/not-complete audit language stay synchronized while evidence is missing or only partially collected; it also requires ready-for-closure status language when all external evidence thresholds are met.
- `tools/external-evidence-check.js` also rejects proxy evidence, incomplete accepted rows, invalid #27 provenance/design-effect vocabulary, #27 experience-map field mismatches, duplicated or mixed explicit no-experience rows, incomplete #33 player route coverage, player route/fixture, phrase, decision, or follow-up mismatches between the ledger and validation results, accepted SME reviews without review-id-linked clean/dirty/missing-control fixture coverage, SME rating, decision, or follow-up mismatches between the ledger and validation results, inconsistent player/SME decisions, missing accepted-evidence document references, missing Fix/Cut follow-up references in validation results, vague, self-referential, or unresolved Fix/Cut follow-up notes, and follow-up count mismatches.
...
- Do not count accepted external-evidence rows unless they pass the ledger validator, document-reference checks, decision-consistency checks, and follow-up issue checks.
...
14. `node tools/external-validation-preflight-check.js` passes.
15. `node tools/external-validation-preflight-check.test.js` passes.
16. `node tools/external-validation-capture-packet-check.js` passes.
17. `node tools/external-validation-capture-packet-check.test.js` passes.
18. `node tools/external-validation-session-handoff.js` passes.
19. `node tools/external-validation-session-handoff.test.js` passes.
20. `node tools/external-validation-execution-contract-check.js --live-issues` passes in an authenticated local `gh` environment.
21. `node tools/external-validation-execution-contract-check.test.js` passes.
22. `node tools/external-validation-intake-runbook-check.js` passes.
23. `node tools/external-validation-intake-runbook-check.test.js` passes.
24. `node tools/external-validation-issue-state-audit.js` passes in an authenticated local `gh` environment.
25. `node tools/external-validation-issue-state-audit.test.js` passes.
26. `node tools/final-closure-readiness-check.js --require-ready` passes.
27. `node tools/final-closure-readiness-check.test.js` passes.
28. `node tools/open-issue-triage-audit.js` passes in an authenticated local `gh` environment.
29. `node tools/open-issue-triage-audit.test.js` passes.
30. `node tools/issue-template-contract-check.js` passes.
31. `node tools/external-validation-full-check.js --live-issues` passes.
32. Main CI is green on the commit that contains the final audit.
