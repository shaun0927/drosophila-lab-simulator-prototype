...
External validation intake staging: [`external-validation-intake-staging.md`](external-validation-intake-staging.md) gives the evidence editor a pre-copy staging sheet for exact phrases, row ids, target docs, decisions, and follow-up dispositions after a real session but before accepted counts are raised. `node tools/external-validation-intake-staging-check.js` verifies the LE/P/SME staging rows and anti-proxy rules; `node tools/external-validation-intake-staging-check.test.js` covers missing-warning, missing-LE-row, wrong-route, wrong-issue, and missing-SME-coverage fixtures.

External validation participant briefs: [`external-validation-participant-briefs.md`](external-validation-participant-briefs.md) provides copy-ready #35 lived-experience, #36-#38 fresh-player, and #39 SME invitation/session-start text without leaking answer keys. `node tools/external-validation-participant-briefs-check.js` verifies invitation scripts, first-30-seconds guardrails, route URLs, SME fixture URLs, and non-evidence warnings; `node tools/external-validation-participant-briefs-check.test.js` covers missing-warning, answer-key leak, wrong URL, missing route row, and missing SME fixture fixtures.
...
External validation capture-packet guardrail: `node tools/external-validation-capture-packet-check.js` verifies the raw capture packet keeps required exact-phrase fields, issue-specific packet links, updated verification commands, SME follow-up warnings, and no-closure language. `node tools/external-validation-capture-packet-check.test.js` covers missing exact phrase, missing full wrapper, missing packet link, and section-order fixtures.

External validation session handoff: `node tools/external-validation-session-handoff.js` prints and verifies the #35-#39 handoff index from execution issue to evidence row, packet source, target docs, and missing proof. `node tools/external-validation-session-handoff.test.js` covers missing tracker row, wrong packet source, missing target doc, and missing packet heading fixtures.
